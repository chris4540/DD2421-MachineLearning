...
class MulticlassSVM:
    ...
    def fit(self, X, y):
        ...
        self._data = X
        self._label = y
        self.alpha = np.zeros((self.n_classes, self.n_samples))
        # self.alpha_times_y = np.zeros((self.n_classes, self.n_samples))
        ...
        for i in range(self.n_samples):
            ...
            truth = self._label_to_custom_onehot(self._label[i])
            for j in range(self.n_classes):
                if inner_prod[j]*truth[j] <= 0:
                    # incorrect prediction, apply delta rule
                    # self.alpha_times_y[j, i] -= preds[j]
                    self.alpha[j, i] -= preds[j]*truth[j]
                    # print(self.alpha_times_y[j, i], self.alpha[j, i]*truth[j])
    ...
    def _inner_prod(self, x):
        ...
        for i in range(self.n_samples):
            kernel_val = self.kernel_fn(x, self._data[i, :])
            # weights = \alpha_{i} y_i
            y_i = self._label_to_custom_onehot(self._label[i])
            weights = self.alpha[:, i]*y_i
